...
class Parser {
  ...
  Parser(Scanner scanner, Speicher speicher, FehlerVerwaltung fehlerVerwaltung, boolean bl) {
    ...
    this.marken = new HashMap<String, Integer>(40);
    this.fixierungen = new HashMap<Integer, String>(80);
                ...
                if (this.aktToken == 3) {
                  ...
                  if (this.erweitert) {
                    if (this.aktToken == 9) {
                      ...
                      this.aktToken = this.scanner.NächstesToken();
                      if (this.aktToken == 2) {
                        if (!this.scanner.BezeichnerGeben().equals("SP")
                            && !this.scanner.BezeichnerGeben().equals("sp")) {
                          this.fehler.FehlerEintragen("'SP' erwartet", this.scanner.PositionGeben());
                        }
                        this.aktToken = this.scanner.NächstesToken();
                      } else {
