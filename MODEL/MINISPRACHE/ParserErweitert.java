...
import MODEL.MINISPRACHE.AblageGeladen;
import MODEL.MINISPRACHE.AblageGlobal;
import MODEL.MINISPRACHE.AblageKonstante;
...
extends Parser {
    ...
    private void Variablenvereinbarung(StackVerwaltung stackVerwaltung) {
        ...
        if (this.aktToken != 2) {
            ...
            do {
                ...
                if (stackVerwaltung == null && this.globaleBezeichner.containsKey(string) || stackVerwaltung != null && this.lokaleBezeichner.containsKey(string)) {
                ...
                if (this.aktToken == 100) {
                    ...
                    if (this.aktToken == 3) {
                        ...
                        if (stackVerwaltung == null) {
                            ...
                            this.lokaleBezeichner.put(string, new LokaleVariable(string, n, stackVerwaltung.OffsetFürNeueVariableGeben(n)));
                        }
                        this.aktToken = this.scanner.NächstesToken();
                    } else {
    ...
    void ParameterSetzen(Prozedur prozedur) {
        ...
        while (true) {
            ...
            if (parameter != null && parameter.IstVarParam()) {
                ...
                } else if (parameter.IstFeld() && ablage.IstFeld() && bezeichner != null && bezeichner instanceof Variable && parameter.FeldlängeGeben() != ((Variable)bezeichner).FeldlängeGeben()) {
                ...
                if (!ablage.IstFeld()) {
                    this.fehler.FehlerEintragen("Einfachen Wert erwartet", this.scanner.PositionGeben());
                } else if (bezeichner != null && bezeichner instanceof Variable && parameter.FeldlängeGeben() != ((Variable)bezeichner).FeldlängeGeben()) {
                    this.fehler.FehlerEintragen("Felder haben unterschiedliche Längen", this.scanner.PositionGeben());
                }
                ...
                for (int i = 0; i < parameter.FeldlängeGeben(); ++i) {
                    ...
                    if (i == parameter.FeldlängeGeben() - 1) continue;
    ...
    private void Zuweisung(Bezeichner bezeichner) {
        ...
        if (this.aktToken == 2 && (bezeichner = this.BezeichnerSuchen(this.scanner.BezeichnerGeben())) != null && bezeichner instanceof Variable) {
        ...
        if (!bl && variable.IstFeld()) {
            ...
            for (int i = 0; i < variable.FeldlängeGeben(); ++i) {
                this.ausgabe.BefehlEintragen(null, "LOAD", "@1(SP)");
                this.ausgabe.BefehlEintragen(null, "STORE", "@0(SP)");
                if (i == variable.FeldlängeGeben() - 1) continue;
                this.ausgabe.BefehlEintragen(null, "LOAD", "1(SP)");
                this.ausgabe.BefehlEintragen(null, "ADDI", "1");
                this.ausgabe.BefehlEintragen(null, "STORE", "1(SP)");
                this.ausgabe.BefehlEintragen(null, "LOAD", "0(SP)");
    ...
    private void VariableAusgeben() {
        for (Bezeichner bezeichner : this.globaleBezeichner.values()) {
            if (!(bezeichner instanceof GlobaleVariable)) continue;
